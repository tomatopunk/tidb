...
func TestSelectNormal(t *testing.T) {
	...
	require.EqualValues(t, 0, response.memTracker.BytesConsumed())
}
...
func TestSelectWithRuntimeStats(t *testing.T) {
	...
	require.ElementsMatch(t, planIDs, response.copPlanIDs, "invalid copPlanIDs")
...
func testNextResponse(t *testing.T, resp SelectResult, chk *chunk.Chunk, requireNumRow int) {
	...
	require.NoError(t, err)
	require.Equal(t, requireNumRow, chk.NumRows())
}
